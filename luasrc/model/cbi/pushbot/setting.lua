...
a=s:taboption("basic", Value,"wechat_corpid",translate('企业ID (CorpID)'), translate("企业微信企业ID").."<br>在企业微信管理后台\"我的企业\"页面获取<br><br>")
a.rmempty = true
a:depends("jsonpath","/usr/bin/pushbot/api/ent_wechat_app.json")

a=s:taboption("basic", Value,"wechat_corpsecret",translate('应用密钥 (Secret)'), translate("企业微信应用的Secret").."<br>在企业微信管理后台\"应用管理\"页面，点击应用查看<br><br>")
a.rmempty = true
a:depends("jsonpath","/usr/bin/pushbot/api/ent_wechat_app.json")

a=s:taboption("basic", Value,"wechat_agentid",translate('应用ID (AgentId)'), translate("企业微信应用的AgentId").."<br>在企业微信管理后台\"应用管理\"页面，点击应用查看<br><br>")
a.rmempty = true
a.datatype = "uinteger"
a:depends("jsonpath","/usr/bin/pushbot/api/ent_wechat_app.json")

a=s:taboption("basic", Value,"wechat_touser",translate('接收用户 (ToUser)'), translate("接收消息的用户ID，多个用户用|分隔").."<br>不填或填写@all表示发送给全部成员<br>用户ID在企业微信管理后台\"通讯录\"中查看<br><br>")
a.rmempty = true
a.default = "@all"
a:depends("jsonpath","/usr/bin/pushbot/api/ent_wechat_app.json")

a=s:taboption("basic", Value,"wechat_media_id",translate('图片ID (Media ID)'), translate("企业微信素材库中的图片ID（选填）").."<br>填写后将发送图文消息，不填则发送纯文本消息<br>需要先通过企业微信API上传图片获取media_id<br>上传接口：https://developer.work.weixin.qq.com/document/path/90253<br><br>")
a.rmempty = true
a:depends("jsonpath","/usr/bin/pushbot/api/ent_wechat_app.json")
